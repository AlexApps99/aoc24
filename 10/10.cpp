...
void aoc_10(std::istream &in, std::string &out1, std::string &out2) {
    ...
    for (uint64_t y = 0; y < input.size(); y++) {
        for (uint64_t x = 0; x < input[y].size(); x++) {
            if (input[y][x] == '0') {
                // count number of "9" positions reachable from starting
                // position, given that we can move horizontally/vertically to
                // the number above the current one (starting at 0)
